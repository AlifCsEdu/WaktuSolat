...
test.describe('Animations & Transitions Automated E2E Verification', () => {
  ...
  test.beforeEach(async ({ page, writeIndexedDB }) => {
    ...
    page.on('console', msg => {
      if (msg.type() === 'error') {
        const text = msg.text();
        // Ignore external offline/network resource load failures
        if (text.includes('Failed to load resource') || text.includes('net::ERR_INTERNET_DISCONNECTED') || text.includes('Flag_of_the_Federal_Territories.svg')) {
          return;
        }
  ...
  test('TR-1: Simulate opening/closing Calendar modal and tab switching', async ({ page }) => {
    ...
    await expect(calendarWrapper).toHaveCount(1); // Navigated page carries only the target modal wrapper
  ...
  test('TR-3: Verify Overlay Priority Queue rendering and sequencing', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      if (typeof (window as any).setUpdateToast === 'function') {
        (window as any).setUpdateToast(true);
      } else {
    ...
    const updateToast = page.locator('div:has-text("Kemaskini")').or(page.locator('div:has-text("Kemas Kini")')).or(page.locator('div:has-text("Update Available")'));
    await expect(updateToast.first()).toBeVisible();

    const locationToast = page.locator('h4:has-text("Lokasi Baru")').or(page.locator('h4:has-text("Lokasi Baharu")')).or(page.locator('h4:has-text("New Location")'));
    await expect(locationToast).not.toBeVisible();

    // Dismiss update-toast
    await page.evaluate(() => {
      if (typeof (window as any).setUpdateToast === 'function') {
        (window as any).setUpdateToast(false);
      } else {
    ...
    const locationToastH4 = page.locator('h4:has-text("Lokasi Baru")').or(page.locator('h4:has-text("Lokasi Baharu")')).or(page.locator('h4:has-text("New Location")')).or(page.locator('h4:has-text("Sila tukar zon")'));
