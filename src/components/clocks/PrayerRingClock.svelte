<script lang="ts">
  ...
  let { movement, todayData = null }: Props = $props();
  ...
  function getPrayerAngle(key: 'fajr' | 'dhuhr' | 'asr' | 'maghrib' | 'isha'): number {
  ...
  let fajrAngle = $derived(getPrayerAngle('fajr'));
  let dhuhrAngle = $derived(getPrayerAngle('dhuhr'));
  let asrAngle = $derived(getPrayerAngle('asr'));
  let maghribAngle = $derived(getPrayerAngle('maghrib'));
  let ishaAngle = $derived(getPrayerAngle('isha'));

  let prayerMarkers = $derived([
...
<div class={cn(
  ...
  {#each prayerMarkers as pr}
    <div class="absolute inset-0 pointer-events-none" style="transform: rotate({pr.angle}deg)">
      <div class={cn(
  ...
  <div class={cn(
    ...
    visualStyle === 'soft' && "bg-[var(--md-sys-color-surface-container-low)] shadow-[var(--soft-shadow-dark)]"
     ...
     <span class={cn(
       ...
       {format(timeState.current, settings.timeFormat === "12h" ? "hh:mm" : "HH:mm")}
     </span>
     {#if settings.timeFormat === "12h"}
       <span class="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-[var(--md-sys-color-on-surface-variant)]/60 -mt-1 select-none font-sans">
         {format(timeState.current, "a")}
       </span>
     {/if}
   </div>
