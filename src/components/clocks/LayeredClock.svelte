<script lang="ts">
  import { appSettings } from "../../state/settings.svelte";
  import { cn } from "../../lib/utils";
  import "@material/web/elevation/elevation.js";

  let { movement = 'sweep' }: { movement?: 'tick' | 'sweep' } = $props();

  let time = $state(new Date());

  $effect(() => {
    if (movement === 'sweep') {
      let frame: number;
      const animate = () => {
        time = new Date();
        frame = requestAnimationFrame(animate);
      };
      frame = requestAnimationFrame(animate);
      return () => cancelAnimationFrame(frame);
    } else {
      const updateTick = () => {
        const now = new Date();
        now.setMilliseconds(0);
        time = now;
      };
      updateTick();
      const interval = setInterval(updateTick, 1000);
      return () => clearInterval(interval);
    }
  });

  let settings = $derived(appSettings.settings);
  let visualStyle = $derived(settings.visualStyle);

  let seconds = $derived(time.getSeconds() + time.getMilliseconds() / 1000);
  let minutes = $derived(time.getMinutes() + seconds / 60);
  let hours = $derived((time.getHours() % 12) + minutes / 60);
</script>

<div class={cn(
  "relative w-full aspect-square shrink-0 mx-auto flex flex-col items-center justify-center transition-all duration-300",
  "w-[90%] sm:w-full max-w-[280px] sm:max-w-[320px] md:max-w-[340px] lg:max-w-[360px]",
  "rounded-full border-[1.5px] border-[var(--md-sys-color-outline-variant)]/20 bg-[var(--md-sys-color-surface-container-lowest)] shadow-inner",
  visualStyle === 'retro' && "border-[3px] border-[var(--md-sys-color-on-surface)] bg-[var(--md-sys-color-surface)] shadow-[6px_6px_0px_0px_var(--md-sys-color-on-surface)] rounded-none",
  visualStyle === 'glass' && "border-2 border-[var(--glass-border)] bg-[var(--glass-bg)]/35 backdrop-blur-[var(--glass-blur)] rounded-full shadow-none",
  visualStyle === 'soft' && "border border-[var(--md-sys-color-outline-variant)]/10 bg-[var(--md-sys-color-surface-container-lowest)] shadow-[var(--soft-shadow-light)] rounded-full"
)}>
  <md-elevation level="2"></md-elevation>
  
  <!-- Outer Ring - Minutes/Seconds Track -->
  <div class={cn(
    "absolute w-[90%] h-[90%] rounded-full border-2 sm:border-[3px] transition-all duration-300",
    "border-[var(--md-sys-color-outline-variant)]/80",
    visualStyle === 'retro' && "border-2 border-[var(--md-sys-color-on-surface)]/60",
    visualStyle === 'glass' && "border-white/25",
    visualStyle === 'soft' && "border-[var(--md-sys-color-outline)]/20"
  )}></div>
  
  <!-- Inner Ring - Hours Track -->
  <div class={cn(
    "absolute w-[60%] h-[60%] rounded-full border-2 sm:border-[3px] transition-all duration-300",
    "border-[var(--md-sys-color-outline-variant)]/60 bg-[var(--md-sys-color-surface-container-low)] shadow-[0_2px_8px_rgba(0,0,0,0.05)]",
    visualStyle === 'retro' && "border-2 border-[var(--md-sys-color-on-surface)] bg-[var(--md-sys-color-surface-container-high)] shadow-none",
    visualStyle === 'glass' && "border-white/15 bg-white/5 backdrop-blur-md",
    visualStyle === 'soft' && "border-[var(--md-sys-color-outline-variant)]/20 bg-[var(--md-sys-color-surface-container-low)] shadow-[var(--soft-shadow-dark)]"
  )}></div>

  <!-- Center dot -->
  <div class={cn(
    "absolute top-1/2 left-1/2 w-4.5 h-4.5 sm:w-5.5 sm:h-5.5 -ml-2.25 sm:-ml-2.75 -mt-2.25 sm:-mt-2.75 rounded-full z-30 shadow-[0_0_8px_var(--md-sys-color-primary)] transition-all",
    "bg-[var(--md-sys-color-primary)]",
    visualStyle === 'retro' && "bg-[var(--md-sys-color-on-surface)] border-2 border-white shadow-none",
    visualStyle === 'glass' && "bg-[var(--md-sys-color-on-surface)] shadow-[0_0_6px_var(--md-sys-color-on-surface)]",
    visualStyle === 'soft' && "bg-[var(--md-sys-color-primary)] shadow-sm"
  )}></div>

  <!-- Hands (Concentric overlapping circles/bars) -->
  <div class="absolute inset-0 pointer-events-none">
    <!-- Hour Hand -->
    <div
      class={cn(
        "absolute top-[28%] left-1/2 w-4 sm:w-5 h-[25%] rounded-full -ml-2 sm:-ml-2.5 origin-[50%_88%] z-10 shadow-md border border-white/20 transition-all",
        "bg-[var(--md-sys-color-secondary)]/85 backdrop-blur-sm",
        visualStyle === 'retro' && "bg-[var(--md-sys-color-primary)] border-2 border-[var(--md-sys-color-on-surface)] rounded-none mix-blend-normal shadow-none w-4.5 sm:w-5.5 -ml-2.25 sm:-ml-2.75",
        visualStyle === 'glass' && "bg-[var(--md-sys-color-on-surface)]/80 border-white/10",
        visualStyle === 'soft' && "bg-[var(--md-sys-color-primary)] border-none"
      )}
      style="transform: rotate({hours * 30}deg)"
    ></div>
    
    <!-- Minute Hand -->
    <div
      class={cn(
        "absolute top-[12%] left-1/2 w-3 sm:w-3.5 h-[42%] rounded-full -ml-1.5 sm:-ml-[1.75px] origin-[50%_90%] z-20 shadow-md border border-white/20 transition-all",
        "bg-[var(--md-sys-color-primary)]/85 backdrop-blur-sm",
        visualStyle === 'retro' && "bg-[var(--md-sys-color-secondary)] border-2 border-[var(--md-sys-color-on-surface)] rounded-none mix-blend-normal shadow-none w-3 sm:w-3.75 -ml-1.5 sm:-ml-[1.875px]",
        visualStyle === 'glass' && "bg-[var(--md-sys-color-on-surface)]/90 border-white/10",
        visualStyle === 'soft' && "bg-[var(--md-sys-color-secondary)] border-none"
      )}
      style="transform: rotate({minutes * 6}deg)"
    ></div>
    
    <!-- Second Hand -->
    <div
      class={cn(
        "absolute top-[5%] left-1/2 w-[2px] sm:w-[2.5px] h-[55%] rounded-full -ml-[1px] sm:-ml-[1.25px] origin-[50%_81.8%] z-30 shadow-sm transition-all",
        "bg-[var(--md-sys-color-error)]",
        visualStyle === 'retro' && "bg-[var(--md-sys-color-error)] w-[2.5px] rounded-none shadow-none",
        visualStyle === 'glass' && "bg-[var(--md-sys-color-error)] shadow-[0_0_8px_var(--md-sys-color-error)]",
        visualStyle === 'soft' && "bg-[var(--md-sys-color-error)]"
      )}
      style="transform: rotate({seconds * 6}deg)"
    ></div>
  </div>
</div>
