<script lang="ts">
  ...
  let words = $derived(timeToWords(time.getHours(), time.getMinutes()));
</script>
...
<div class={cn(
  ...
  {#if visualStyle === 'default'}
    <div class="absolute top-0 right-0 w-1/2 h-1/2 bg-[var(--md-sys-color-primary)]/[0.04] rounded-full blur-3xl pointer-events-none"></div>
  {/if}
  {#if visualStyle === 'glass'}
    <div class="absolute bottom-0 left-0 w-2/3 h-2/3 bg-[var(--md-sys-color-tertiary)]/5 rounded-full blur-3xl pointer-events-none"></div>
  {/if}

  <!-- Fluid word layout filling the aspect-square grid -->
  <div class="font-black uppercase tracking-tighter whitespace-pre-line select-none font-sans flex-1 flex flex-col justify-center gap-0.5 sm:gap-1.5 relative z-10 min-w-0 overflow-hidden">
    {#each words.split('\n') as line}
      <div class={cn(
