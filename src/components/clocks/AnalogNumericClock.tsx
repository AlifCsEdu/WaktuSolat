...
const NUMERIC_POSITIONS = [
  ...
  { x: 17.1, y: 69, num: 8 }, // 8
  { x: 12, y: 50, num: 9 },   // 9
  { x: 17.1, y: 31, num: 10 },// 10
  { x: 31, y: 17, num: 11 },  // 11
];

export function AnalogNumericClock({ movement }: { movement: 'tick' | 'sweep' }) {
  ...
  return (
    <div className={cn(
      ...
      {/* Numeric Hour Markers */}
      {NUMERIC_POSITIONS.map((pos) => (
        <div
          key={pos.num}
          className={cn(
            ...
            visualStyle === 'retro' && "text-[var(--md-sys-color-on-surface)] font-black drop-shadow-none",
            visualStyle === 'glass' && "text-[var(--md-sys-color-on-surface)] drop-shadow-md",
