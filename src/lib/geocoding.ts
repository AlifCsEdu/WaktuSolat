...
/**
 * Find the closest JAKIM zone to the given coordinates by computing Haversine distances
 * to known zone centroid coordinates. Used as an instant offline GPS lookup fallback.
 */
export function findNearestZone(lat: number, lng: number): string {
  ...
  for (const [zone, coords] of Object.entries(ZONE_COORDINATES)) {
    const dist = calculateHaversineDistance(lat, lng, coords[0], coords[1]);
    if (dist < minDistance) {
      minDistance = dist;
      nearestZone = zone;
    }
